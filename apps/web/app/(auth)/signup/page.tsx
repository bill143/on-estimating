    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
          ...
          <p className="text-sm text-zinc-500 mb-1">We sent a confirmation link to</p>
          <p className="text-sm font-medium text-zinc-700 mb-6">{successEmail}</p>
          <p className="text-xs text-zinc-400">Click the link in the email to confirm your account, then sign in.</p>

          <Link href="/login" className="inline-block mt-6 text-sm text-orange-600 hover:text-orange-700 font-medium hover:underline">
            Back to Sign In
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="space-y-1.5">
            ...
            <Input id="name" type="text" placeholder="Jane Smith" autoComplete="name" {...register('name')} className="border-zinc-300 focus-visible:ring-orange-500" />
            {errors.name && <p className="text-sm text-red-600">{errors.name.message}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="email" className="text-zinc-700">Email</Label>
            <Input id="email" type="text" placeholder="you@company.com" autoComplete="email" {...register('email')} className="border-zinc-300 focus-visible:ring-orange-500" />
        ...
        <p className="mt-6 text-center text-sm text-zinc-500">
          ...
          <Link href="/login" className="text-orange-600 hover:text-orange-700 font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
