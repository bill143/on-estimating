    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="bg-gray-900 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h2 className="text-sm font-bold text-white tracking-wide uppercase shrink-0">Estimate Summary Sheet</h2>

          {/* Collapsed inline values */}
          {collapsed && (
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="flex h-6 w-6 items-center justify-center rounded text-gray-400 hover:bg-gray-700 hover:text-white transition-colors"
      ...
      <div
        ...
        style={{ maxHeight: collapsed ? 0 : 800 }}
      >
        <div className="p-5 grid grid-cols-12 gap-x-6 gap-y-3">
          ...
          <div className="col-span-12">
            <div className="border-t border-gray-100 pt-3 mt-1">
              <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-2">Scope & Bid Notes</label>

              <div className="grid grid-cols-12 gap-x-6 gap-y-3">
                <div className="col-span-12">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Scope</label>
                  <textarea
                    rows={3}
                    value={header.scope}
                    onChange={(e) => setHeader({ scope: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg bg-gray-50 text-sm text-gray-700 focus:border-brand-500 focus:outline-none p-2 resize-none"
                    placeholder="Describe the scope of work for this estimate..."
                  />
                </div>
                <div className="col-span-6">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Exclusions</label>
                  <textarea
                    rows={2}
                    value={header.exclusions}
                    onChange={(e) => setHeader({ exclusions: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg bg-gray-50 text-sm text-gray-700 focus:border-brand-500 focus:outline-none p-2 resize-none"
                    placeholder="Items excluded from this estimate..."
                  />
                </div>
                <div className="col-span-6">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Clarifications</label>
                  <textarea
                    rows={2}
                    value={header.clarifications}
                    onChange={(e) => setHeader({ clarifications: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg bg-gray-50 text-sm text-gray-700 focus:border-brand-500 focus:outline-none p-2 resize-none"
                    placeholder="Assumptions and clarifications..."
                  />
                </div>
              </div>
            </div>
