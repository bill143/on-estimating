...
function MarkdownRenderer({ content }: { content: string }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('### ')) {
      elements.push(<h3 key={i} className="text-base font-bold text-zinc-800 mt-6 mb-2">{line.slice(4)}</h3>);
    } else if (line.startsWith('## ')) {
      elements.push(<h2 key={i} className="text-lg font-bold text-zinc-900 mt-8 mb-3 pb-2 border-b border-zinc-100">{line.slice(3)}</h2>);
    } else if (line.startsWith('# ')) {
      elements.push(<h1 key={i} className="text-xl font-bold text-zinc-900 mb-4">{line.slice(2)}</h1>);
    } else if (line.startsWith('---')) {
      ...
      elements.push(
        <div key={i} className="flex items-start gap-2 my-1 ml-2">
          ...
          <span className={cn('text-sm', checked ? 'text-zinc-400 line-through' : 'text-zinc-700')}>{line.slice(6)}</span>
        </div>,
      );
    } else if (line.startsWith('- ')) {
      elements.push(
      ...
      elements.push(<p key={i} className="text-xs text-zinc-400 italic my-2">{line.replace(/^\*|\*$/g, '')}</p>);
    } else {
      elements.push(<p key={i} className="text-sm text-zinc-700 leading-relaxed my-1">{renderInline(line)}</p>);
    }
  }
  flushTable();
